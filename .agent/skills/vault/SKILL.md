...
### Step 1：Vault 健康報告
用 tsx 掃描 `{VAULT_PATH}/GetThreads/`：
- 分類分布統計
- 最近 7 天新增
- 空資料夾偵測
...
掃描 GetThreads Vault 筆記，偵測 6 種品質問題並自動修復。
...
2. 產生 `GetThreads/知識庫摘要.md`（統計 + 核心實體 + 關鍵洞察）
3. 產生 `GetThreads/知識地圖.md`（分類列表 + 關聯圖 + 缺口）
4. 產生 Claude Code 命令（若有高密度主題）
