...
export function assembleNote(
  ...
  // 3. Main body (platform-specific)
  const { text: bodyText, usedPaths } = formatter.formatBody(displayText, imageUrlMap);
  lines.push(bodyText, '');
  ...
  // 8. Videos (platform-specific)
  const videoLines = formatter.formatVideos(content.videos, localVideoPaths);
  if (videoLines.length > 0) {
    lines.push('## Videos', '', ...videoLines);
  }

  // 9. Stats + Comments (shared)
  lines.push(...buildStats(content.likes, content.reposts));
