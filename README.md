...
| **查詢 Query** | Vault 知識問答、主題探索、深度合成、對比分析、語意搜尋、互動式研究、衝突偵測、缺口地圖 | `/ask` / `/explore` / `/distill` |
| **驗證 Validate** | 雙層創業驗證：AI 市場快照（競品 + 趨勢）+ 3 個 AI 無法代勞的人工判斷問題 | `/validate <想法>` |
...
- **知識蒸餾** — `/distill` 普通蒸餾；`/distill conflicts` 衝突偵測（掃描矛盾觀點與對立洞察）；`/distill gaps` 缺口地圖（高頻實體缺乏深探 / 分類覆蓋不足 / 洞察孤島，過濾通用詞雜訊後列出最值得補強的主題）
- **想法驗證** — `/validate <想法>` 雙層創業驗證：AI 自動掃描 Vault 競品與趨勢 → 生成市場快照 + 差異化切入點；輸出 3 個只有你能回答的核心判斷問題，明確區分 AI 可分析與人工必須判斷的邊界
- **值班效率** — `/patrol stats` Bot 主動採集比例報告，以 2026-05-10 為追蹤基線，統計 patrol / radar / 訂閱自動存檔占比，評估 Bot 自主值班效率
...
| | `/distill` | 知識蒸餾（`conflicts` 衝突偵測 / `gaps` 缺口地圖） |
| | `/validate <想法>` | 雙層創業驗證（AI 市場快照 + 3 個人工判斷問題） |
...
| | `/patrol stats` | Bot 主動採集效率報告（追蹤基線 2026-05-10） |
...
| `/distill` | `conflicts` | 衝突偵測（掃描矛盾觀點 + 對立洞察） |
| | `gaps` | 缺口地圖（高頻實體 / 薄弱分類 / 洞察孤島） |
| `/validate` | `<想法>` | 雙層驗證：AI 市場快照 + 3 個人工判斷問題 |
...
- **知識系統** — 實體萃取、知識圖譜、衝突偵測（矛盾關係 + 對立洞察）、缺口地圖（高頻實體 / 薄弱分類 / 洞察孤島，停用詞過濾）、Skill 自動生成、用戶偏好萃取、知識蒸餾（含視覺化提示詞）、記憶整合、MOC 生成、Karpathy 主題編譯；Vault 分析器從 frontmatter `summary` 自動生成洞察（99.6% 覆蓋率）
...
│   ├── vault-analyzer.ts       # 增量 Vault 分析（實體萃取 + 關係抽取 + 洞察自動生成）
│   ├── conflict-analyzer.ts    # 衝突偵測（矛盾關係 + 對立洞察掃描）
│   ├── gap-analyzer.ts         # 缺口地圖（高頻實體 / 薄弱分類 / 洞察孤島，停用詞過濾）
...
│   └── proactive-stats.ts      # Bot 主動採集比例統計（追蹤基線 2026-05-10）
