---
title: auto-skill
description: "開發型任務的底層依賴（研究型任務跳過）。每次對話自動檢查知識庫並調用最佳實踐；建立跨技能經驗記憶層，讓下次使用同技能時能主動提醒。當用戶表達滿意時，必須詢問是否記錄經驗。"
---
...
- 否則必須執行以下步驟：
  ...
  4. **僅在找到有實質內容的經驗時**提示：`我已讀取經驗：skill-xxx.md`（空索引或無新內容則靜默）
  5. 若 `experience/_index.json` 沒有該技能，記錄到 `missing_experience_skills`（不輸出提示）
...
- 詢問語句需明確指向該技能，例如：
  > 「這次使用了 health-check，但經驗庫沒有紀錄。我可以把這次的做法記錄下來嗎？」
...
所有路徑相對於 auto-skill 技能目錄（`~/.Codex/skills/auto-skill/`）：
