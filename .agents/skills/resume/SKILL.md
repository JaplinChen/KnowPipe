...
# 2. 殭屍進程
pgrep -cf "node.*src/index" 2>/dev/null || echo 0
...
# 5. Vault 7 天新增
VAULT_PATH=$(grep VAULT_PATH .env | cut -d'=' -f2 | sed 's/^"//;s/"$//')
find "$VAULT_PATH/ObsBot" -name "*.md" -mtime -7 -type f 2>/dev/null | wc -l
