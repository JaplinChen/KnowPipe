...
/launch --diagnose   # 深度診斷（進程分析 → 精確 kill → 網路連線 → 重啟）
...
### Phase 2：清除殭屍進程
```bash
# 精確匹配 Bot 進程（不殺 Claude Code 或 VS Code 的 node）
pgrep -f 'src/index.ts|dist/index.js' | xargs kill -TERM 2>/dev/null
sleep 3
# 確認已停止
pgrep -f 'src/index.ts|dist/index.js' >/dev/null && echo "⚠️ 進程未停止" || echo "✅ 清除完成"
```

### Phase 3：清除 Lockfiles
```bash
rm -f .bot.pid .bot.lock bot.pid
```

### Phase 4：TypeScript 編譯
```bash
npx tsc --noEmit
```
有錯誤 → **停止**。
...
額外清除 lockfile（.bot.pid、.bot.lock、bot.pid）。
...
### Step 1：進程分析
```bash
ps -eo pid,ppid,lstart,command | grep -E 'node|tsx' | grep -v grep
```
判斷進程用途：bot 相關 → kill | claude/vscode → 保留

### Step 2：Lockfile + 網路連線
```bash
lsof -i -n | grep -E '149\.154|91\.108'
```
...
### Step 1：列出 Bot 進程
```bash
ps -eo pid,command | grep -E 'src/index.ts|dist/index.js' | grep -v grep
```
...
sleep 2
# SIGKILL survivors
pgrep -f 'src/index.ts|dist/index.js' | xargs kill -9 2>/dev/null
```
